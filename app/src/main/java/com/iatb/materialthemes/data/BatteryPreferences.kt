...
object BatteryPreferences {
    private const val PREFS_NAME = "battery_widget_preferences"
    private const val KEY_FOLLOW_CLOCK_WEATHER = "follow_clock_weather"
    private const val KEY_BATTERY_COLOR_PALETTE = "battery_color_palette"
    private const val KEY_BATTERY_CUSTOM_COLOR = "battery_custom_color"
    private const val KEY_BATTERY_TRANSPARENCY = "battery_transparency"
    ...
    fun getColorPalette(context: Context): ColorPalette {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val name = prefs.getString(KEY_BATTERY_COLOR_PALETTE, ColorPalette.DYNAMIC.name)
        return try {
    ...
    fun setColorPalette(context: Context, palette: ColorPalette) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putString(KEY_BATTERY_COLOR_PALETTE, palette.name).apply()
    }

    fun getCustomColor(context: Context): Int {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        return prefs.getInt(KEY_BATTERY_CUSTOM_COLOR, 0xFF00796B.toInt())
    }

    fun setCustomColor(context: Context, color: Int) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putInt(KEY_BATTERY_CUSTOM_COLOR, color).apply()
    }

    fun getTransparency(context: Context): Int {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        return prefs.getInt(KEY_BATTERY_TRANSPARENCY, 100)
    }

    fun setTransparency(context: Context, transparency: Int) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putInt(KEY_BATTERY_TRANSPARENCY, transparency.coerceIn(0, 100)).apply()
    }
