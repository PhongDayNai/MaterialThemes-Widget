...
class WidgetPresetEditActivity : AppCompatActivity() {
    ...
    private fun checkAndSavePreset() {
        ...
        // Add Mode: prompt user for preset name
        val suggestions = getPresetNameSuggestions(category, palette)
        val input = EditText(this).apply {
            hint = getString(R.string.dialog_add_preset_hint)
            isSingleLine = true
            maxLines = 1
            filters = arrayOf(InputFilter.LengthFilter(MAX_PRESET_TITLE_LENGTH))
        }
        ...
        val container = LinearLayout(this).apply {
            ...
            if (suggestions.isNotEmpty()) {
                val suggestionsView = buildSuggestionsLayout(suggestions) { selectedName ->
                    input.setText(selectedName)
                    input.setSelection(selectedName.length)
                    input.error = null
                }
                ...
                addView(suggestionsView, scrollParams)
            }
        }
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (name.length > MAX_PRESET_TITLE_LENGTH) {
                input.error = getString(R.string.preset_name_too_long_warning, MAX_PRESET_TITLE_LENGTH)
                input.requestFocus()
                return@setOnClickListener
            }
    ...
    private fun showEditPresetNameDialog() {
        ...
        val currentTitle = editingPresetTitle ?: ""
        val input = EditText(this).apply {
            hint = getString(R.string.dialog_add_preset_hint)
            isSingleLine = true
            maxLines = 1
            ...
            if (currentTitle.isNotEmpty()) {
                setSelection(currentTitle.length.coerceAtMost(MAX_PRESET_TITLE_LENGTH))
            }
        }
        ...
        val dialog = MaterialAlertDialogBuilder(this)
            .setTitle(R.string.dialog_edit_preset_name_title)
            .setView(container)
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (newName.length > MAX_PRESET_TITLE_LENGTH) {
                input.error = getString(R.string.preset_name_too_long_warning, MAX_PRESET_TITLE_LENGTH)
                input.requestFocus()
                return@setOnClickListener
            }
            if (WidgetPreferences.isPresetTitleTaken(this, newName, excludeId = editingPresetId)) {
                input.error = getString(R.string.preset_duplicate_name_warning, newName)
                input.requestFocus()
                return@setOnClickListener
            }
    ...
    private fun buildSuggestionsLayout(
        ...
        if (suggestions.size <= 2) {
            val singleRow = LinearLayout(this).apply {
            ...
            for (suggestedName in suggestions) {
                val chip = createSuggestionChip(suggestedName) {
            ...
            for (name in itemsRow1) {
                val chip = createSuggestionChip(name) {
                    onSelected(name)
                }
                val chipParams = LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    private fun distributeSuggestionsIntoTwoRows(
        ...
        // Fixed width of chip: paddingStart(10dp) + paddingEnd(12dp) + icon(14dp) + iconMargin(5dp) + marginEnd(8dp) = 49dp
        ...
        val countRow2 = n / 2
        val countRow1 = n - countRow2 // countRow1 >= countRow2

        var bestRow2Indices: List<Int>? = null
        var bestScore = Float.MAX_VALUE

        // Recursively find the combination of countRow2 items that yields minimal difference (Row1 >= Row2)
        fun findCombinations(start: Int, chosen: MutableList<Int>) {
            if (chosen.size == countRow2) {
                ...
                val diff = w1 - w2
                // Prioritize Row1 >= Row2 with minimal difference; penalize heavily if Row2 > Row1
        ...
        // When countRow1 > countRow2 (odd n), pick the shortest item in Row1 as the tail so the trailing part is minimal
        ...
        // Pair items column-by-column to keep cumulative width differences minimal throughout the scroll
    ...
    private fun getPresetNameSuggestions(cat: WidgetCategory, pal: ColorPalette): List<String> {
        val catName = when (cat) {
            WidgetCategory.DIAGONAL -> getString(R.string.preset_short_cat_diagonal)
            WidgetCategory.ORGANIC -> getString(R.string.preset_short_cat_organic)
            WidgetCategory.SCALLOP -> getString(R.string.preset_short_cat_scallop)
        }
        val palName = when (pal) {
            ColorPalette.DYNAMIC -> getString(R.string.preset_short_pal_dynamic)
            ColorPalette.OLIVE -> getString(R.string.preset_short_pal_olive)
            ColorPalette.TEAL -> getString(R.string.preset_short_pal_teal)
            ColorPalette.SLATE -> getString(R.string.preset_short_pal_slate)
            ColorPalette.AMBER -> getString(R.string.preset_short_pal_amber)
            ColorPalette.CRIMSON -> getString(R.string.preset_short_pal_crimson)
            ColorPalette.CUSTOM -> getString(R.string.preset_short_pal_custom)
        }
        val pixelStr = getString(R.string.preset_expr_pixel)

        val expr1 = when (pal) {
            ColorPalette.DYNAMIC -> getString(R.string.preset_expr_dynamic_1)
            ColorPalette.OLIVE -> getString(R.string.preset_expr_olive_1)
            ColorPalette.TEAL -> getString(R.string.preset_expr_teal_1)
            ColorPalette.SLATE -> getString(R.string.preset_expr_slate_1)
            ColorPalette.AMBER -> getString(R.string.preset_expr_amber_1)
            ColorPalette.CRIMSON -> getString(R.string.preset_expr_crimson_1)
            ColorPalette.CUSTOM -> getString(R.string.preset_expr_custom_1)
        }
        val expr2 = when (pal) {
            ColorPalette.DYNAMIC -> getString(R.string.preset_expr_dynamic_2)
            ColorPalette.OLIVE -> getString(R.string.preset_expr_olive_2)
            ColorPalette.TEAL -> getString(R.string.preset_expr_teal_2)
            ColorPalette.SLATE -> getString(R.string.preset_expr_slate_2)
            ColorPalette.AMBER -> getString(R.string.preset_expr_amber_2)
            ColorPalette.CRIMSON -> getString(R.string.preset_expr_crimson_2)
            ColorPalette.CUSTOM -> getString(R.string.preset_expr_custom_2)
        }
        val expr3 = when (pal) {
            ColorPalette.DYNAMIC -> getString(R.string.preset_expr_dynamic_3)
            ColorPalette.OLIVE -> getString(R.string.preset_expr_olive_3)
            ColorPalette.TEAL -> getString(R.string.preset_expr_teal_3)
            ColorPalette.SLATE -> getString(R.string.preset_expr_slate_3)
            ColorPalette.AMBER -> getString(R.string.preset_expr_amber_3)
            ColorPalette.CRIMSON -> getString(R.string.preset_expr_crimson_3)
            ColorPalette.CUSTOM -> getString(R.string.preset_expr_custom_3)
        }
        val expr4 = when (pal) {
            ColorPalette.DYNAMIC -> getString(R.string.preset_expr_dynamic_4)
            ColorPalette.OLIVE -> getString(R.string.preset_expr_olive_4)
            ColorPalette.TEAL -> getString(R.string.preset_expr_teal_4)
            ColorPalette.SLATE -> getString(R.string.preset_expr_slate_4)
            ColorPalette.AMBER -> getString(R.string.preset_expr_amber_4)
            ColorPalette.CRIMSON -> getString(R.string.preset_expr_crimson_4)
            ColorPalette.CUSTOM -> getString(R.string.preset_expr_custom_4)
        }
    ...
    private fun createSuggestionChip(suggestedName: String, onSelected: () -> Unit): View {
        val density = resources.displayMetrics.density
        val chip = LinearLayout(this).apply {
            ...
            val bgPill = GradientDrawable().apply {
                ...
                setColor(getThemeColor(com.google.android.material.R.attr.colorSurfaceVariant, 0xFFE0E0E0.toInt()))
                setStroke((1 * density).toInt(), getThemeColor(com.google.android.material.R.attr.colorOutline, 0xFFBDBDBD.toInt()))
            }
            background = bgPill
            isClickable = true
            isFocusable = true
        ...
        val ivIcon = ImageView(this).apply {
            ...
            imageTintList = android.content.res.ColorStateList.valueOf(
                getThemeColor(androidx.appcompat.R.attr.colorPrimary, 0xFF006874.toInt())
            )
        }
        ...
        val tvText = TextView(this).apply {
            text = suggestedName
            textSize = 13f
            setTextColor(getThemeColor(androidx.appcompat.R.attr.colorPrimary, 0xFF006874.toInt()))
        }
