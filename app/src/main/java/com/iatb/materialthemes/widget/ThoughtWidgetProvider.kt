...
import com.iatb.materialthemes.render.ThoughtWidgetRenderer
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class ThoughtWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        const val ACTION_UPDATE_THOUGHT_WIDGET = "com.iatb.materialthemes.action.UPDATE_THOUGHT_WIDGET"

        fun updateAllWidgets(context: Context) {
            val appWidgetManager = AppWidgetManager.getInstance(context) ?: return
            val component = ComponentName(context, ThoughtWidgetProvider::class.java)
            val ids = try {
        ...
        suspend fun renderWidget(
            ...
            val state = stateManager.getState(appWidgetId)
                ?: stateManager.createInitialState(appWidgetId, ContentType.THOUGHT)

            val content = state.contentId?.let { repository.getContentById(it) }
                ?: repository.getFallbackContent(ContentType.THOUGHT, state.languageFilter)
